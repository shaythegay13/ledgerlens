 ...
 */
export async function generateLedgerLensReport(input: ModelInput, provider: ProviderConfig & { model: LanguageModel } = requireAnalysisModel()): Promise<{ report: AiAnalysisReport; attempts: number; provider: ProviderConfig }> {
  let rejection = "";
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try { return { report: validateAiAnalysis(result.output, input), attempts: attempt, provider }; }
